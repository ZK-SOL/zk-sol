...
export async function processAndValidateTransaction(instructions: TransactionInstruction[], connection: Connection, signer: Keypair) {
	...
	assert.equal(sig.SignatureResult.err, null, `${txn?.meta?.logMessages.join('\n')}\n\n${JSON.stringify(sig)}`);
}
...
export async function getTxn(program: Program<any>, signature: string): Promise<ParsedTransactionWithMeta | null> {
	const blockStats = await program.provider.connection.getLatestBlockhash();
	const strategy: BlockheightBasedTransactionConfirmationStrategy = {
		signature: signature,
		blockhash: blockStats.blockhash,
...
export async function getTokenAccountBalance(connection: Connection, account: PublicKey): Promise<number> {
	const account_info = await connection.getAccountInfo(account);
	if (account_info === null) {
		return 0;
	}
	const tokenBalance = await connection.getTokenAccountBalance(account, 'confirmed');
	return parseInt(tokenBalance.value.amount);
}

export async function getWalletBalance(connection: Connection, wallet: PublicKey, mint: PublicKey): Promise<number> {
