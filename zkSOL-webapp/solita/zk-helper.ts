...
export async function run_circuit({
    ...
    const nullifierHashBigInt = CryptoHelper.numberArrayToBigInt(nullifierHashBytes);
    ...
    const publicSignalsBuffer_1 = CryptoHelper.to32ByteBuffer(BigInt(publicSignals[1]));
    let public_signal_0_u8_array_1 = Array.from(publicSignalsBuffer_1);
    assert(CryptoHelper.compareArrays(recipientNum.mod, [...public_signal_0_u8_array_0]))
    assert(CryptoHelper.compareArrays(nullifierHashBytes, [...public_signal_0_u8_array_1]))
...
export class ZkHelper {
    ...
    static convertProofToBytes(proof: ZKProof): Uint8Array {
        ...
        return CryptoHelper.concatenateUint8Arrays([
            ...pi_a,
            ...pi_b,
            ...pi_c
        ]);
